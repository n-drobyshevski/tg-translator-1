...
def entities_to_html(text: str, entities: List[Any] | None) -> str:
    ...
    for ent in entities_sorted:
        ...
        if ent.type == MessageEntityType.BOLD:
            ...
            # Ignorer ou ajouter d’autres cas…
            continue

        # 4. Insérer balises (dans la chaîne échappée)
        #   On découpe la chaîne et on injecte
...
class MetadataRequest:
    def __init__(self, chat_id: int, message_id: int, file_id: str | None = None):
        ...
        self.response: asyncio.Future[Dict[str, Any]] = asyncio.get_event_loop().create_future()
...
def init_clients(env: Dict[str, str]) -> Tuple[Client, Application, Anthropic, TelegramSender]:
    pyro = Client(
        "bot",
...
def build_prompt(html_text: str, channel: str, link: str) -> str:
    ...
    intro = "Translate the following HTML message without duplicating original message:" if short else ""
...
def register_handlers(pyro: Client, anthropic: Anthropic, sender: TelegramSender, mapping: Dict[int, str]):
    ...
    async def handle_message(_: Client, msg):
        ...
        req.message_entities = [e.to_dict() if hasattr(e, "to_dict") else e for e in raw_entities]
